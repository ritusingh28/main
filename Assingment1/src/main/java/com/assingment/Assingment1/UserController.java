...
public class UserController {
    ...
    @PostMapping("/bulk-register")
    public String bulkRegisterUsers(@RequestParam("file") MultipartFile file, RedirectAttributes redirectAttributes) {
        try {
            userService.bulkRegisterUsersFromCsv(file.getInputStream());
            redirectAttributes.addFlashAttribute("message", "Bulk registration completed successfully");
        } catch (Exception e) {
            redirectAttributes.addFlashAttribute("error", "Error processing the file: " + e.getMessage());
        }
        return "redirect:/webapp/users/getAll";
    }
    @GetMapping("/bulk-register")
    public String showBulkRegisterForm() {
        // Return the name of the Thymeleaf template for the bulk registration form
        return "bulk-register";
    }
